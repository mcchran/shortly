...
from shorty.logger import logger
...
def fallback_chain(func1, func2):
    """Generates a chain of fallback retries

    :param func func1: the preferred function to go with
    :param func func2: the fallback function
...
class SecretNotFoundError(Exception):
    """ Should be raised whenever the reqyested
    secret is not found in the vault
    """
    pass


def get_secret(secret_name):
    """Should the corresponding secret
    We assume that the secret is stored into some kind
    of vault.
    In our case we consider vault to be the os environmental
    variables
